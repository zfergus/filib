...
char* AllZerosErrMsg (int Err)
{
   ...
   switch (Err) {
      case NoError:      break;
      case IllMaxZeroNo: snprintf(Msg,sizeof(Msg),"Error: Parameter for maximum number of zeros must lie in 1,...,%1d!", MaxCount);
                         break;
      case NotAllZeros:  snprintf(Msg,sizeof(Msg),"Warning: Not all zeros found due to the user limit of %1d zero(s)!",MaxZeroNo);
                         break;
      default:           snprintf(Msg,sizeof(Msg),"Error Code not defined");
   }
